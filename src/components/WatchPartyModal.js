...
const WatchPartyModal = ({ content = {}, onClose, onCreateParty, darkMode }) => {
  ...
  // Set default values for content if it's undefined
  const contentTitle = content?.title || 'Movie';
  const contentGenre = content?.genre || 'Drama';
  const contentLanguage = content?.language || 'English';
  const contentPosterUrl = content?.posterUrl || 'https://via.placeholder.com/300x200?text=Movie';
        ...
        {showCode ? (
          ...
          <>
            <div className="flex mb-6">
                ...
                {contentPosterUrl ? (
                  <img 
                    ...
                    onError={(e) => {
                      ...
                      e.target.src = "https://via.placeholder.com/300x200?text=Movie";
                    }}
                  />
                ) : (
              ...
              <div>
                <h3 className="text-xl font-bold">{contentTitle}</h3>
                <p className={`${darkMode ? 'text-gray-300' : 'text-gray-600'}`}>{contentGenre}</p>
                <p className={`${darkMode ? 'text-gray-300' : 'text-gray-600'}`}>{contentLanguage}</p>
            ...
            <form onSubmit={handleSubmit}>
              <div className="mb-4">
                ...
                <input
                  ...
                  placeholder={`${contentTitle} Party`}
                />
              </div>
